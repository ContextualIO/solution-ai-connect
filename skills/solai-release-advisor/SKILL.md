...
- `sourceTenantId` **present** → **installed** (the service was installed from another tenant — operating on it means acting against a *target* tenant, often production). **This build does not include a target-tenant workflow.** Target-tenant operations should use least-privilege credentials and follow your organization's policy for AI use in production or otherwise sensitive tenants; the install/update and its review remain in the workspace UI. Surface this and stop — do not improvise an audit.
