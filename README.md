...
- `solai-knowledge-mcp` now performs OAuth discovery from the server itself and delegates registration/token issuance to Stytch Connected Apps, so the bundled `.mcp.json` entry stays URL-only even though first use requires sign-in.
