...
- `solai-knowledge-mcp` is OAuth-protected: the bundled `.mcp.json` entry is URL-only, and the server advertises OAuth discovery and token issuance (via Stytch) on the `401` challenge. Requires an MCP client that supports OAuth discovery.
